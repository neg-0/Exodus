...
public class TerrainManager {
	 ...
	 */
	public List<Countable<Integer>> loadSchematic(File saveFile, Location loc) throws FilenameException, DataException, IOException, MaxChangedBlocksException, EmptyClipboardException {
		saveFile = we.getSafeSaveFile(localPlayer,
		                              saveFile.getParentFile(), saveFile.getName(),
		                              EXTENSION, new String[] { EXTENSION });
		...
		return localSession.getClipboard().getBlockDistribution();
	}
	
	/**
	 * Load the data from the given schematic file and paste it at the given location with no air.  If the location is null, then
	 * paste it at the saved data's origin.
	 * 
	 * @param saveFile	a File representing the schematic file to load
	 * @param loc		the location to paste the clipboard at (may be null)
	 ...
	 * @throws EmptyClipboardException
	 */
	public List<Countable<Integer>> loadSchematicNoAir(File saveFile, Location loc) throws FilenameException, DataException, IOException, MaxChangedBlocksException, EmptyClipboardException {
		saveFile = we.getSafeSaveFile(localPlayer,
		                              saveFile.getParentFile(), saveFile.getName(),
		                              EXTENSION, new String[] { EXTENSION });
		...
		editSession.enableQueue();
		localSession.setClipboard(SchematicFormat.MCEDIT.load(saveFile));
		localSession.getClipboard().place(editSession, getPastePosition(loc), true);
		editSession.flushQueue();
		we.flushBlockBag(localPlayer, editSession);
		return localSession.getClipboard().getBlockDistribution();
	}
